...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=user,text=f"<b>📌Nᴀᴍᴇ ➠ : <code>{files.file_name}</code> \n\n🔗ꜰɪʟᴇ ꜱɪᴢᴇ ➠ : {get_size(files.file_size)}</b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('📂 Dᴏᴡɴʟᴏᴀᴅ Nᴏᴡ 📂', url=g)
                    ], [
        ...
        for file in files:
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                     [
                      ...
                      InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url='https://t.me/moviesprimiumworld')
                   ],[
                      InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/Oneplus5999")
                     ],
                     [
                        InlineKeyboardButton('🚀 Fast Download / Watch Online🔥', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @LazyDeveloperr
            ...
            filesarr.append(msg)
        k = await client.send_message(chat_id = message.from_user.id, text=f"<b><u>⚠️⚠️⚠️ɪᴍᴘᴏʀᴛᴀɴᴛ⚠️⚠️⚠️</u></b>\n\n𝗧𝗵𝗶𝘀 𝗠𝗼𝘃𝗶𝗲 𝗙𝗶𝗹𝗲/𝗩𝗶𝗱𝗲𝗼 𝘄𝗶𝗹𝗹 𝗯𝗲 𝗱𝗲𝗹𝗲𝘁𝗲𝗱 𝗶𝗻  <b><u>10 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there</i></b>")
    ...
    if not files_:
        ...
        try:
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                ...
                await message.reply_text(
                    text="<b>You are not verified !\nKindly verify to get direct files for the next 24hrs 😇 How to Verify?- https://t.me/Tiger_3_Animal_12th_fail/63 !</b>",
                    protect_content=True,
                    reply_markup=InlineKeyboardMarkup(btn)
                )
                return
            ...
            k = await msg.reply("<b><u>⚠️⚠️⚠️ɪᴍᴘᴏʀᴛᴀɴᴛ⚠️⚠️⚠️</u></b>\n\n𝗧𝗵𝗶𝘀 𝗠𝗼𝘃𝗶𝗲 𝗙𝗶𝗹𝗲/𝗩𝗶𝗱𝗲𝗼 𝘄𝗶𝗹𝗹 𝗯𝗲 𝗱𝗲𝗹𝗲𝘁𝗲𝗱 𝗶𝗻  <b><u>10 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there</i></b>",quote=True)
            await asyncio.sleep(600)
            await msg.delete()
            await k.edit_text("<b>ʏᴏᴜʀ ꜰɪʟᴇ/ᴠɪᴅᴇᴏ ɪꜱ ᴅᴇʟᴇᴛᴇᴅ\n\nᴄʟɪᴄᴋ ʙᴇʟᴏᴡ ᴛᴏ 👇</b>",reply_markup=InlineKeyboardMarkup(btn))
            return
        except:
            pass
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@Oneplus5999  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
