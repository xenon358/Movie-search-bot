...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ᴄʜᴇᴄᴋ ɪɴ ᴏᴜʀ <a href=https://t.me/Tiger_3_Animal_12th_fail>ᴍᴏᴠɪᴇ ᴄʜᴀɴɴᴇʟ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>",   
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/Prime_Movieshd")]])
    )
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        await query.answer("🚩 𝗡𝗼 𝗙𝗶𝗹𝗲 𝗪𝗲𝗿𝗲 𝗙𝗼𝘂𝗻𝗱 - ᴅᴍ ᴏᴡɴᴇʀ ᴛᴏ ᴀᴅᴅ ꜰɪʟᴇꜱ @Razer358 🚩", show_alert=1)
        return
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("⛩️𝗚ᴇᴛ ᴀʟʟ ꜰɪʟᴇꜱ ɪɴ 1 ᴄʟɪᴄᴋ⛩️", callback_data=f"sendfiles#{key}"),
    ...
    # don't change anything without contacting me @LazyDeveloperr
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/Razer358")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<b>⚡𝗧𝗶𝘁𝗹𝗲 : {search}\n\n📝 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n\n⏰ ʀᴇsᴜʟᴛ ɪɴ : {remaining_seconds} sᴇᴄᴏɴᴅs\n\n Powered By/nMovie Search Bot</b>"
        else:
            cap = f"<b>⚡𝗧𝗶𝘁𝗹𝗲 : {search}\n\n📝 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n\n⏰ ʀᴇsᴜʟᴛ ɪɴ : {remaining_seconds} sᴇᴄᴏɴᴅs\n\n Powered By/nMovie Search Bot</b>"
